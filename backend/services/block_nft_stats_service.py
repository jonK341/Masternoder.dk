"""Block Smiley NFT — AI-style battle stats + arena rewards (plan 004)."""
...
_CONFIG_PATH = os.path.join(_BASE, "data", "block_nft_battle_config.json")
_HISTORY_PATH = os.path.join(_BASE, "data", "block_nft_battle_history.json")
_COOLDOWN_PATH = os.path.join(_BASE, "data", "block_nft_battle_cooldowns.json")
...
def simulate_battle(attacker: Dict[str, Any], defender: Dict[str, Any]) -> Dict[str, Any]:
    """Resolve a single NFT battle round."""
    seed = _seed(
...
def _battles_today(user_id: str, edition_key: str) -> int:
    ...
    doc = _read_json(_COOLDOWN_PATH, {"days": {}})
...
def battle_with_nft(user_id: str, edition_key: str) -> Dict[str, Any]:
    """Battle using an owned Block Smiley NFT edition; award unified points."""
    cfg = get_config()
    ...
    try:
        ...
        if bp_delta:
            unified_points_db.add_points(uid, "battle_points", float(bp_delta), "block_nft_battle", meta)
        if gp_delta:
            unified_points_db.add_points(uid, "game_points", float(gp_delta), "block_nft_battle", meta)
        if result == "win":
            unified_points_db.add_points(uid, "battle_wins", 1, "block_nft_battle", meta)
    except Exception:
        pass
