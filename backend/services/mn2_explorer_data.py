...
def _parse_richlist_payload(raw: Any, limit: int) -> List[Dict[str, Any]]:
...
def rich_list_index_synced() -> bool:
    """True when eiquidus richlist/wealth distribution index is populated."""
    dist = _explorer_http_get("/getdistribution", ttl=_RICH_TTL)
    if isinstance(dist, dict) and dist.get("supply"):
        tier = dist.get("t_1_25")
        if isinstance(tier, dict) and tier.get("total") is not None:
            return True
    return bool(rich_list(limit=1))
...
def rich_list(limit: int = 100) -> List[Dict[str, Any]]:
    ...
    rows: List[Dict[str, Any]] = []
    for path in (
        f"/getrichlist?limit={limit}",
        f"/getrichlistbalance?limit={limit}",
    ):
        raw = _explorer_http_get(path, ttl=_RICH_TTL)
        rows = _parse_richlist_payload(raw, limit)
        if rows:
            break
    with _LOCK:
...
def explorer_status() -> Dict[str, Any]:
    ...
    synced = rich_list_index_synced()
    rich_ok = synced
    checks["rich_list"] = {
        "ok": rich_ok,
        "sample_count": len(rich) if rich else (1 if synced else 0),
        "index_synced": synced,
