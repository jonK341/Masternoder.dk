...
def _wallet_entry_from_address(address: str, label: str = "primary") -> Dict[str, Any]:
    """Build a full multi-address wallet record from a single deposit address."""
...
def _normalize_wallet_entry(entry: Any) -> Optional[Dict[str, Any]]:
    """Upgrade legacy bare address strings to full wallet dicts."""
    if isinstance(entry, str):
        addr = entry.strip()
        return _wallet_entry_from_address(addr) if addr else None
    if isinstance(entry, dict):
        primary = (entry.get("primary") or "").strip()
        rows = entry.get("addresses")
        if primary or (isinstance(rows, list) and rows):
            if not primary:
                first = rows[0] if rows else None
                if isinstance(first, dict):
                    primary = (first.get("address") or "").strip()
                elif isinstance(first, str):
                    primary = first.strip()
            if primary and not isinstance(rows, list):
                entry = dict(entry)
                entry["primary"] = primary
                entry.setdefault("wallet_type", "core")
                entry["addresses"] = [{
                    "label": "primary",
                    "address": primary,
                    "type": "core",
...
def normalize_all_legacy_wallets() -> Dict[str, Any]:
    """Upgrade every bare address string in mn2_user_addresses.json to a full wallet record."""
    addresses = _load_addresses()
    upgraded = 0
    for key, entry in list(addresses.items()):
        if isinstance(entry, str) and entry.strip():
            addresses[key] = _wallet_entry_from_address(entry.strip())
            upgraded += 1
    if upgraded:
        _save_addresses(addresses)
    return {"success": True, "upgraded": upgraded, "total_entries": len(addresses)}


def ensure_user_wallet(user_id: str) -> Dict[str, Any]:
    """Best-effort: each user gets a full wallet record with labeled addresses. Never raises."""
    try:
        ...
        if entry is not None:
            if isinstance(entry, str):
                entry = _wallet_entry_from_address(entry.strip())
                addresses[uid] = entry
                _save_addresses(addresses)
            primary = _entry_primary(entry)
            if primary and _address_validity(primary) is not False:
                wtype = entry.get("wallet_type") if isinstance(entry, dict) else "core"
                return {
                    "success": True,
                    "user_id": uid,
            ...
            addresses.pop(uid, None)
        created = get_or_create_deposit_address(uid)
        if created.get("success"):
            entry = _load_addresses().get(uid)
            created["wallet_type"] = (
                entry.get("wallet_type") if isinstance(entry, dict) else "core"
            )
        return created
...
def ensure_user_deposit_address(user_id: str) -> Dict[str, Any]:
    """Alias for ensure_user_wallet (backward compatible)."""
    return ensure_user_wallet(user_id)
...
def get_or_create_deposit_address(user_id: str) -> Dict[str, Any]:
    ...
    if user_id in addresses:
        entry = addresses[user_id]
        if isinstance(entry, str):
            entry = _wallet_entry_from_address(entry)
            addresses[user_id] = entry
            _save_addresses(addresses)
        existing = _entry_primary(entry)
        if existing and _address_validity(existing) is not False:
            wtype = entry.get("wallet_type") if isinstance(entry, dict) else "core"
            return {
                "success": True,
                "deposit_address": existing,
                "user_id": user_id,
                "wallet_type": wtype,
            }
    ...
    if pool_addr:
        addresses[user_id] = _wallet_entry_from_address(pool_addr)
        _save_addresses(addresses)
        return {
            "success": True,
            "deposit_address": pool_addr,
            "user_id": user_id,
            "wallet_type": "core",
        }
