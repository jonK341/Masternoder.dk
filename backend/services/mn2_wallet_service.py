...
_VALIDATION_TIMEOUT_SEC = 3.0
_GENERATION_TIMEOUT_SEC = 8.0


def _wallet_generation_available(timeout_sec: float = _VALIDATION_TIMEOUT_SEC) -> tuple:
    """Return (ready, user_error). Probes wallet RPC without creating addresses."""
    try:
        from backend.services.mn2_rpc_client import getwalletinfo
        r = getwalletinfo(timeout_sec=timeout_sec)
    ...
    if r.get("error"):
        ...
        if "method not found" in low and "disabled" in low:
            return False, "Wallet is still loading on the network node. Try again in a few minutes."
        if any(x in low for x in ("connection refused", "timed out", "timeout", "unreachable", "failed to establish")):
            return False, "Wallet RPC unreachable. Ensure the MN2 wallet node is running."
        return False, err
    return True, None


def _address_validity(addr: str, timeout_sec: Optional[float] = None) -> Optional[bool]:
    ...
    if not addr or not isinstance(addr, str):
        return False
    probe_timeout = _VALIDATION_TIMEOUT_SEC if timeout_sec is None else timeout_sec
    try:
        from backend.services.mn2_rpc_client import validateaddress
        r = validateaddress(addr.strip(), timeout_sec=probe_timeout)
    except Exception:
        return None
    if r.get("error"):
        return None
...
def _generate_valid_address(max_attempts: int = 3) -> Dict[str, Any]:
    ...
    ready, ready_err = _wallet_generation_available()
    if not ready:
        return {"success": False, "error": ready_err or "Wallet RPC not ready for address generation"}
    from backend.services.mn2_rpc_client import getnewaddress
    last_err = None
    for _ in range(max(1, max_attempts)):
        r = getnewaddress(timeout_sec=_GENERATION_TIMEOUT_SEC)
        if r.get("error"):
            last_err = r["error"]
            break
        addr = r.get("result")
        if not addr or not isinstance(addr, str):
            ...
            continue
        addr = addr.strip()
        if _address_validity(addr, timeout_sec=_VALIDATION_TIMEOUT_SEC) is False:
...
def get_or_create_deposit_address(user_id: str) -> Dict[str, Any]:
    ...
    # a *definitive* invalid when RPC can mint a replacement; an RPC outage (None)
    # or a resyncing wallet must keep the existing address so profile stays usable.
    if user_id in addresses:
        existing = _entry_primary(addresses[user_id])
        if not existing:
            addresses.pop(user_id, None)
        else:
            validity = _address_validity(existing)
            ...
            if not ready:
                return {
                    ...
                    "user_id": user_id,
                    "address_warning": ready_err or "Wallet node is resyncing; deposits may not credit until a fresh address is issued.",
                }
            addresses.pop(user_id, None)
...
def create_deposit_addresses(count: int) -> Dict[str, Any]:
    ...
    if not ready:
        return {
            ...
            "created": [],
            "count": 0,
            "error": ready_err or "Wallet RPC not ready for address generation",
        }
