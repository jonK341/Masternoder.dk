...
@health_bp.route('/api/health/gate-b', methods=['GET'])
def gate_b_health():
    """Gate B readiness: economy core services (ledger, pricing, rewards, wallet)."""
    try:
        from backend.services.gate_b_status_service import check_gate_b
        result = check_gate_b()
        status_code = 200 if result.get('ready_for_stage_2') else 503
...
@health_bp.route('/api/health/gate-d', methods=['GET'])
def gate_d_health():
    """Gate D readiness: Stage 3 cross-cutting (monitor, security cron, control board)."""
    try:
        from backend.services.gate_d_status_service import check_gate_d
        result = check_gate_d()
        status_code = 200 if result.get('ready_for_stage_4') else 503
        return jsonify(result), status_code
    except Exception as exc:
        return jsonify({
            'success': False,
