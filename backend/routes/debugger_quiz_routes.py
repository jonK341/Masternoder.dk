"""Debugger Top-50 Q&A quiz — server-side MN2 rewards (Phase 6)."""
...
def _score_reward(correct: int, total: int) -> float:
    ...
    ratio = correct / total
    if correct < _MIN_CORRECT or ratio < _MIN_RATIO:
        return 0.0
...
def submit_quiz(
    ...
    *,
    ...
    append_entry(
        ...
        amount=amount,
        txid=reference,
        metadata={"correct": int(correct), "total": int(total), "day": quiz_day},
    )
    emit(
    ...
    # Soft XP on the debugger quiz tab (non-money).
    try:
        unified_points_db.add_points(
            user_id,
            "activity_points",
            25,
            source="debugger_quiz_tab",
            metadata={"reference": f"debugger_quiz_xp:{user_id}:{quiz_day}", "tab": "quiz"},
        )
    except Exception:
        pass
