...
def _toggle_distribution_stream(stream_id: str, enable: bool):
    from backend.services.monetization_streams_service import reload_streams
    import json as _json

    path = os.path.join(
        os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
        "data",
        "monetization_streams.json",
    )
    try:
        with open(path, "r", encoding="utf-8") as f:
            doc = _json.load(f)
        ...
        found = False
        for s in streams:
            if isinstance(s, dict) and s.get("id") == stream_id:
                s["enabled"] = enable
                s["status"] = "live" if enable else "paused"
                found = True
                break
        if not found:
            return jsonify({"success": False, "error": "stream_not_found", "stream_id": stream_id}), 404
        with open(path, "w", encoding="utf-8") as f:
            _json.dump(doc, f, indent=2)
        reload_streams()
        return jsonify({"success": True, "stream_id": stream_id, "enabled": enable}), 200
    except Exception as exc:
        return jsonify({"success": False, "error": str(exc)}), 500


@monetization_expansion_bp.route("/api/monetization/enable/<stream_id>", methods=["POST"])
def monetization_enable_stream(stream_id: str):
    return _toggle_distribution_stream(stream_id, True)


@monetization_expansion_bp.route("/api/monetization/disable/<stream_id>", methods=["POST"])
def monetization_disable_stream(stream_id: str):
    return _toggle_distribution_stream(stream_id, False)
