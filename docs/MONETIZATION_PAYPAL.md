...
| Usage vs allowance (habit loops) | `backend/services/monetization_allowance_service.py`, `GET /api/monetization/usage-allowance`, cron `monetization_allowance_nudges` |
...
| Step | Action | Status |
|------|--------|--------|
| 1 | **Subscription SKUs.** Recurring billing aligned to **included generations** (or reference-equivalent credits) per month; **overage** priced **above** blended COGS from metering (never below). | **Partial — shipped:** PayPal **Subscriptions** end-to-end; shop **Pro** block; **`overage-pack-2`** / **`overage-pack-4`** + **`GET /api/monetization/overage-offers`** + shop PayPal buttons + generator **402 upsell**. **Remaining:** real **plan ids**, optional **Stripe**; tune overage prices from metering p90. |
| 2 | **Credit packs + reference honesty.** Sell **generation credits** tied to **reference fractions** (`credit_definition.reference_fraction_per_credit` in **`data/monetization_config.json`**) so prices stay defensible when **job mix** shifts. | **Partial — shipped:** packs + **`generation_credits_granted`** on packs; **`GET /api/monetization/config`**; shop shows **`reference_eq_label`** on coin packs. **Remaining:** revisit fraction when the **canonical reference job** changes ([REFERENCE_JOB_COGS.md](./REFERENCE_JOB_COGS.md)). |
| 3 | **Habit loops.** Before renewal: **in-app** or **email** — e.g. “You used **~X%** of your monthly allowance (metered vs cap).” Reduces **surprise churn** and supports **fair upsell**. | **Partial — shipped:** **`GET /api/monetization/usage-allowance`**, shop **PayPal & coins** banner, cron job **`monetization_allowance_nudges`** → **`logs/monetization/allowance_nudges.jsonl`**. **Remaining:** email/scheduler wiring; profile surface. |
...
| § | Topic | Code / product |
|---|--------|----------------|
| §1 | Coin / credit packs | **Partial** — **`data/monetization_config.json`** is the single source for pack SKUs + **`credit_definition.reference_fraction_per_credit`** + optional **`generation_credits_granted`** per pack; `GET /api/monetization/config` and **`GET /api/shop/coin-packs`** expose **`reference_eq_label`**. Live PayPal + shop UX remains ops. |
...
| §8.2–8.5 | Retention, infra, B2B API, growth | **Shipped (repo):** usage-allowance + overage PayPal + allowance/renewal email cron (`monetization_allowance_nudges`, `monetization_renewal_emails`); shop promo codes (`WELCOME10`); SCR self-serve deposit; paid queue priority (`monetization_priority_service`); premium per-render PayPal; battle pass; metered API keys (`X-Generator-Api-Key` on generate-video); Camgirls PayPal unlock/tip. **Ops:** `NOTIFY_SMTP_*`, live PayPal plan ids, `GENERATOR_API_KEY_SECRET`. |
...
| **Top-10 rails** | Overage, SCR, queue, premium render, battle pass, API keys, Camgirls PayPal, promos, emails | **Shipped in repo** — see `backend/routes/monetization_expansion_routes.py` + shop/camgirls/generator UI hooks. **Remaining ops:** live credentials, cron schedule, battle-pass coin deduction (coins button grants without spend — wire to shop wallet if needed). |
