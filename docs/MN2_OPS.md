...
| Rule | Detail |
|------|--------|
| **Config** | `data/mn2_config.json` → `agent_funding`: `{ per_agent_mn2: 100000, trader_agent_count: 6, top_up: true, live_distribute: false }`. **live_distribute defaults false** (Option C — dry-run/status only). |
...
| **Status (safe)** | `GET /api/agents/treasury/status` — dry-run gaps, pool balance, gates; never moves funds. |
| **Distribution gate** | `POST /api/agents/treasury/distribute` is dry-run while `live_distribute=false`. Live mode requires `live_distribute=true` **and** `X-Ops-Secret` **and** recorded sign-off in `data/treasury_signoff.json` before any batch ≥100k MN2. |
...
Until sign-off is recorded **and** `agent_funding.live_distribute` is flipped to `true`, `distribute_agent_funding()` stays dry-run / returns `treasury_signoff_required` for batches ≥100k MN2.
