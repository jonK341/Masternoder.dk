...
| **P1** | **Exchange leveling + adaptive learning + super skills** — trader XP/achievements, agent game time/levels, profit-based skill proficiency, Sentient Apex bot | **Code** | exchange marketplace | **Done 2026-07-06** — `exchange_leveling_service` + leveling strip in command center (XP bar, rank, fee discount, claim hint); `crypto_exchange_bp` now registered in full blueprint pass. |
| **P1** | **Trust system + Live Watch + activations** — user/agent trust tiers, composite IQ, manual activation gates, owner review console | **Code** | exchange leveling | **Done 2026-07-06** — email/Discord trust alerts (`check_and_emit_trust_alerts`), Gold auto-activate (`auto_activate_gold`), trust badge in command center, `/api/exchange/trust/check-alerts` + `/api/exchange/trust/auto-activate`. |
| **P1** | **Bridge high-value product** — cross-quests, fused leaderboard, Discord market pulse | **Code** | user controller | **Done 2026-07-06** — cross-quest UI + High-roller table links added to casino tab in command center hub. |
| **P1** | **Exchange gateway — PayPal webhooks** | **Code** | gateway layer | **Done 2026-07-06** — `exchange_paypal_webhook_service` + `/api/exchange/paypal/webhook`; verifies PayPal signature, auto-fulfills crypto/MN2 orders on PAYMENT.CAPTURE.COMPLETED without buyer online. |
| **P1** | **Live price feed** | **Code** | exchange criticals | **Done 2026-07-06** — `exchange_live_price_service` + `/api/exchange/prices/live` + `/api/exchange/prices/refresh`; aggregates mid-prices across venues, writes drift to `price_cache.json`; master daemon refreshes every tick. |
| **P1** | **Exchange daemon systemd unit** | **Code** | exchange criticals | **Done 2026-07-06** — `systemd/exchange_master_daemon.service` + `scripts/install_exchange_daemon.sh`; run `sudo bash scripts/install_exchange_daemon.sh` on server. |
...
| **P2** | **Sync local `DEPLOY_PASS`** — non-interactive SSH on legacy remote scripts | Win | **Done 2026-07-06** | `mn2_masternode_start_fleet_remote.py`, `mn2_masternode_reinstall_remote.py`, `mn2_enable_staking.py` migrated to `connect_deploy_ssh()` (SSH key first, password fallback). All key remote scripts now use unified auth. |
