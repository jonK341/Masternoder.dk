# Loose Ends Plan - MasterNoder.dk  <!-- pragma: allowlist secret -->
...
This document outlines all identified loose ends in the MasterNoder.dk project and provides a comprehensive plan to address them systematically.  <!-- pragma: allowlist secret -->
...
**Issues:**
- Unclear if `player_levels`, `xp_history`, `daily_activities` tables exist in production  <!-- pragma: allowlist secret -->
- `rewards` and `user_rewards` tables need to be created
- Database migration status unknown

**Actions:**
- [ ] Run `scripts/migrate_hunters_game_complete.py` on production  <!-- pragma: allowlist secret -->
