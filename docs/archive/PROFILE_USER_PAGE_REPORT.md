...
- Active session/device list is now centralized in `data/account_sessions.json` and supports non-current revocation. A database-backed session store would still be stronger for production scale.  <!-- pragma: allowlist secret -->
...
- Move OAuth token/session storage from local JSON into the production database or secret store before relying on it for high-value accounts.  <!-- pragma: allowlist secret -->
