...
**Primær prod-URL i repo:** `https://masternoder.dk` — afstem **masternodes.dk** vs **masternoder.dk** i DNS/panel hvis begge nævnes.  <!-- pragma: allowlist secret -->
...
|------|----------|--------|
| **DNS `masternoder.dk`** | **OK** | `A` → `140.82.39.124` (PowerShell `Resolve-DnsName`) |  <!-- pragma: allowlist secret -->
| **DNS `masternodes.dk`** | **Fejl** | *Navnet på fjernenheden kunne ikke fortolkes* — intet A/AAAA fra denne maskine |
| **`GET https://masternoder.dk/api/health`** | **200** | Svar ~77 bytes |  <!-- pragma: allowlist secret -->
| **`GET https://masternoder.dk/api/health/system`** | **Timeout** | 25s — ingen respons i tid |  <!-- pragma: allowlist secret -->
| **`GET https://masternoder.dk/api/health/database`** | **503** | HTTP 503 |  <!-- pragma: allowlist secret -->
...
| **D** | DNS `masternoder.dk` | Resolverer | **OK** | `A` → `140.82.39.124`. |  <!-- pragma: allowlist secret -->
...
| Prioritet | Item |
|-----------|------|
| **P0** | Afklar **masternodes.dk** vs **masternoder.dk** (DNS/panel). |  <!-- pragma: allowlist secret -->
...
| URL | Primært `https://masternoder.dk`; bekræft **masternodes.dk** separat. |  <!-- pragma: allowlist secret -->
| Repo | `Masternoder.dk` |  <!-- pragma: allowlist secret -->
