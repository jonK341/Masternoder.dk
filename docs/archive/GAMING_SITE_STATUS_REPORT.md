...
- Continue product tuning after deployment: real seasonal persistence, richer reward art, and production analytics dashboards.  <!-- pragma: allowlist secret -->
- Continue account hardening after deployment: move OAuth/session/recovery JSON storage into production-grade persistence, configure real recovery email delivery, run browser tests, and finish the legacy `innerHTML` audit.  <!-- pragma: allowlist secret -->

## Verification Notes

- `python scripts/smoke_pages_game_battle.py --only-html` passed against `https://masternoder.dk`: `/game` and `/battle` both returned 200.  <!-- pragma: allowlist secret -->
- `PLATFORM_BASE_URL=https://masternoder.dk PLATFORM_SKIP_SLOW_CHECKS=1 python scripts/service_check_all_components.py` passed, including Star Map 25 data/status and key pages.  <!-- pragma: allowlist secret -->
