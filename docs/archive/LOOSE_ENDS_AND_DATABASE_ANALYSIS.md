...
**Issue:** Database models exist (`PlayerLevel`, `XPHistory`, `DailyActivity`) but it's unclear if tables are created in production.  <!-- pragma: allowlist secret -->

**Evidence:**
- Models defined in `src/db/models.py` (referenced in docs)
- Migration script exists: `scripts/migrate_hunters_game.py`
- No verification that tables exist in production database  <!-- pragma: allowlist secret -->
