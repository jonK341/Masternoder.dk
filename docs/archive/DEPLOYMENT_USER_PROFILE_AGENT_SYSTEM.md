...
1. **Upload Files**
   ...
   scp -r backend/services/user_*.py root@masternoder.dk:/var/www/html/vidgenerator/backend/services/  <!-- pragma: allowlist secret -->
   scp backend/routes/user_profile_routes.py root@masternoder.dk:/var/www/html/vidgenerator/backend/routes/  <!-- pragma: allowlist secret -->
   scp backend/register_blueprints.py root@masternoder.dk:/var/www/html/vidgenerator/backend/  <!-- pragma: allowlist secret -->
   scp vidgenerator/static/js/agent-dashboard-data.js root@masternoder.dk:/var/www/html/vidgenerator/static/js/  <!-- pragma: allowlist secret -->
   # ... upload dashboard files
   ```

2. **Create Log Directories**
   ```bash
   ssh root@masternoder.dk  <!-- pragma: allowlist secret -->
...
```bash
# Test user creation
curl -X POST http://masternoder.dk/vidgenerator/api/user/create \  <!-- pragma: allowlist secret -->
  -H "Content-Type: application/json" \
  -d '{"device_fingerprint": "test123"}'

# Test agent skills
curl http://masternoder.dk/vidgenerator/api/user/agent-skills/test_user_1  <!-- pragma: allowlist secret -->

# Test agent controller
curl http://masternoder.dk/vidgenerator/api/agents/controller/status  <!-- pragma: allowlist secret -->
```
...
- [ ] Files deployed to production  <!-- pragma: allowlist secret -->
- [ ] Services restarted
...
**Deployment Status:** Ready for production deployment  <!-- pragma: allowlist secret -->
