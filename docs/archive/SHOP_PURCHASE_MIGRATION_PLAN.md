...
- [x] Test purchase flow (unit + API line checks; production migration is ops-owned)  <!-- pragma: allowlist secret -->
- [x] Update frontend (`shop/index.html` v4 — inventory + purchase history, Owned badges, product line + API checks)
- [ ] Run SQL migration on production (optional if file-mode is acceptable; use `GET /api/shop/payment-health` → `shop_storage` for mode)  <!-- pragma: allowlist secret -->
...
| **Ops** | Production may stay on file-mode or migrate once; check `shop_storage` in payment-health. |  <!-- pragma: allowlist secret -->
