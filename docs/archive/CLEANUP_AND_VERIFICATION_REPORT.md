...
### Registration Status
- ✅ `production_debugger_bp` - Registered in `register_blueprints.py` (line 75)  <!-- pragma: allowlist secret -->
- ✅ `system_aggregator_bp` - Registered in `register_blueprints.py` (line 86)
- ✅ Route files exist on server:
  - `/var/www/html/backend/routes/production_debugger_routes.py` ✓  <!-- pragma: allowlist secret -->
...
```python
@production_debugger_bp.route('/api/debug/system/<system_name>', methods=['GET'])  <!-- pragma: allowlist secret -->
@production_debugger_bp.route('/vidgenerator/api/debug/system/<system_name>', methods=['GET'])  <!-- pragma: allowlist secret -->
```
