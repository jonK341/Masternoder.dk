...
**Target:** https://masternoder.dk/vidgenerator    <!-- pragma: allowlist secret -->
...
**Cause:** Routes are not registered on production (python-proxy/uwsgi PYTHONPATH or deployment not updated).  <!-- pragma: allowlist secret -->
...
| Generator APIs 404 on production | Deploy latest code, ensure PYTHONPATH and services (python-proxy, uwsgi-vidgenerator) are correct |  <!-- pragma: allowlist secret -->
...
1. **Deploy:** Run `scripts/deploy_vidgenerator_solution.py` and `scripts/ensure_flask_routes_registered.py` on production.  <!-- pragma: allowlist secret -->
...
**Problem:** Production returned JSON `{"error":"Not Found","message":"Resource not found","path":"/","success":false}` for `/` and “more sites” (vidgenerator pages) because no route served `/` or `/vidgenerator`.  <!-- pragma: allowlist secret -->
...
```bash
python scripts/test_vidgenerator_all_pages.py --base https://masternoder.dk  <!-- pragma: allowlist secret -->
python scripts/test_vidgenerator_all_pages.py --local   # against http://127.0.0.1:5000
```

**Deploy:** `deploy_vidgenerator_solution.py` now includes `backend/routes/all_page_routes.py`. After deploy, run the test script against production to confirm all pages return 200.  <!-- pragma: allowlist secret -->
