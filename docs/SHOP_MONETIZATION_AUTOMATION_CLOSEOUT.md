...
See **`SHOP_PURCHASE_MIGRATION_PLAN.md`** (conclusion table at bottom). Summary: SQL migration is **optional** for production if **`shop_file_mode`** is acceptable; **`GET /api/shop/payment-health`** includes **`shop_storage`** for visibility.
