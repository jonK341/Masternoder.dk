...
def payout_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(pay, "_treasury_stashed_usd_by_mode", lambda mode: 200.0 if str(mode).lower() == "paper" else 0.0)
    monkeypatch.setattr(pay, "_sweep_ledger_mode", lambda: "paper")
