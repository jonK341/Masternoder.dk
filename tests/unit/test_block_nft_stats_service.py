"""Plan 004 — block smiley NFT battle stats and arena."""
...
def test_generate_battle_stats_deterministic():
    from backend.services.block_nft_stats_service import generate_battle_stats, serial_number
    ...
    assert a["nft_kind"] == "block_smiley"


def test_simulate_battle_produces_result():
    from backend.services.block_nft_stats_service import generate_battle_stats, simulate_battle
...
def test_battle_with_nft_rewards():
    from backend.services import block_nft_stats_service as bns
    ...
    edition = {
        ...
        "edition_no": 1,
        "block_height": 2000,
        "battle_stats": bns.generate_battle_stats(2000, "TRO-block-2000-1", edition_no=1),
    }
    ...
    with tempfile.TemporaryDirectory() as tmp:
        with patch.object(bns, "_HISTORY_PATH", os.path.join(tmp, "hist.json")):
            with patch.object(bns, "_COOLDOWN_PATH", os.path.join(tmp, "cd.json")):
                with patch.object(bns, "_find_owned_edition", return_value=edition):
                    with patch("backend.services.unified_points_database.unified_points_db", mock_points):
                        result = bns.battle_with_nft("alice", "TRO-block-2000-1")
