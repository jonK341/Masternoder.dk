#!/usr/bin/env python3
"""Deploy route fix - comment out conflicting route"""
import paramiko
...
def deploy_files():
    ...
    try:
        ...
        for file_path in FILES_TO_DEPLOY:
            ...
            # Normalize path for remote server (Linux uses forward slashes)
            remote_file_path = file_path.replace('\\', '/')
            remote_path = os.path.join(REMOTE_BASE, remote_file_path).replace('\\', '/')
