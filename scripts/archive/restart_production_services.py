"""
Restart all production services after deployment  # pragma: allowlist secret
"""
import os
import sys
import paramiko
...
SERVER_HOST = os.getenv("DEPLOY_HOST", "masternoder.dk")  # pragma: allowlist secret
USERNAME = os.getenv("DEPLOY_USER", "root")
PASSWORD = (os.getenv("DEPLOY_PASS") or "").strip() or (_ for _ in ()).throw(SystemExit("Set DEPLOY_PASS for SSH."))

def restart_production_services():  # pragma: allowlist secret
    """Restart all production services"""  # pragma: allowlist secret
    ssh_client = paramiko.SSHClient()
    ssh_client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    
    try:
        print("=" * 80)
        print("RESTARTING PRODUCTION SERVICES")  # pragma: allowlist secret
        print("=" * 80)
...
if __name__ == '__main__':
    success = restart_production_services()  # pragma: allowlist secret
