#!/usr/bin/env python3
"""Check MasterNoder2 v1.2.3.0 release readiness (tag, GitHub asset, local tarball)."""
...
def main() -> int:
    p = argparse.ArgumentParser(description="MN2 v1.2.3.0 release gate checks")
    ...
    pr = _gh_json(["pr", "view", "1", "--repo", REPO, "--json", "state,mergedAt,title"])
    if pr:
        print(f"PR #1: {pr.get('state')} ({pr.get('mergedAt', '')[:10]}) — {pr.get('title')}")
    else:
        print("PR #1: (gh unavailable)")

    tag = _gh_json(["api", f"repos/{REPO}/git/refs/tags/{TARGET_VERSION}"])
    print(f"Git tag {TARGET_VERSION}: {'yes' if tag else 'MISSING'}")
    ...
    if published_ready:
        ...
        print("  1. Build:    bash scripts/mn2_build_release.sh  (or mn2_build_release_remote.py --ask-pass)")
        print(
        ...
        print("  5. Upgrade:  python scripts/mn2_daemon_upgrade_remote.py --ask-pass --apply")
