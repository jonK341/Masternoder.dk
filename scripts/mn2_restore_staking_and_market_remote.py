...
'''
    ...
    print("Running on server (wallet unlock + 3 market ticks — may take 1–3 min, output appears when done)...\n")
    out = sh(ssh, remote, timeout=300)
    print(out)
    ssh.close()

    ok = "staking_after= True" in out or "staking_after=True" in out
    print("\n" + ("Staking + market restore completed." if ok else "[WARN] Staking may still be off — check output."))
