#!/usr/bin/env python3
"""Remote MN2 daemon version audit + v1.2.3.0 upgrade helper (when release assets exist)."""
...
def main() -> int:
    ...
    if args.check_release:
        if not asset_ok:
            ...
            print("  bash scripts/mn2_build_release.sh")
            print(
                "  python scripts/mn2_publish_release.py --tarball dist/masternoder2d.tar.gz "
                "--manifest dist/RELEASE_MANIFEST.json --draft"
            )
            return 1
        return 0

    if args.apply and not asset_ok:
        print("ERROR: Cannot --apply until v1.2.3.0 release asset exists on GitHub.", file=sys.stderr)
    ...
    if args.apply:
        ...
        print("=== upgrade v1.2.3.0 (stop → backup wallet → fetch → verify → install → start) ===")
        script = f"""
set -e
