"""Deploy quick-load changes (stream HTML, async CSS, defer scripts) then restart Flask."""
import paramiko
import os
import sys
...
def main():
    ...
    print("Deploying quick-load changes to", REMOTE_BASE)
    for f in FILES:
    ...
    sftp.close()
    ssh.close()
    print("\nDeploy done. Running restart_flask_app...")
    import subprocess
    ok = subprocess.call(
