#!/usr/bin/env python3
"""
Deploy Frontend Regeneration - Updated HTML files with cache busting
"""
import paramiko
import os
import sys
...
# Updated HTML files with new cache versions
UPDATED_HTML_FILES = [
...
ALL_FILES = UPDATED_HTML_FILES + NEW_STATIC_FILE

def deploy_frontend():
    """Deploy regenerated frontend files"""
    print("=" * 70)
    print("Frontend Regeneration - Production Deployment")
    print("=" * 70)
    ...
    try:
        ...
        # Deploy files
        print("[2/5] Deploying regenerated frontend files...")
        sftp = ssh.open_sftp()
        deployed = 0
        skipped = 0
        errors = 0
        ...
        print()
        print("=" * 70)
        print("✅ Frontend Regeneration Complete!")
        print("=" * 70)
        print()
        print("Deployed Files:")
        print(f"  - {len(UPDATED_HTML_FILES)} HTML files with new cache versions")
        print(f"  - {len(NEW_STATIC_FILE)} new static file")
        print()
        print("Next Steps:")
        print("  1. Visit: https://masternoder.dk/vidgenerator/")
        print("  2. Hard refresh: Ctrl+F5 (Windows) or Cmd+Shift+R (Mac)")
        print("  3. Check browser console (F12) for any errors")
        print("  4. Verify agent data appears in all dashboards")
        print()
